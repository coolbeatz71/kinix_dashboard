...
export interface IListStoryProps {
    onTitle?: (title: string) => void;
    onSelect?: (story: IUnknownObject) => void;
}

const ListStory: FC<IListStoryProps> = ({ onSelect, onTitle }) => {
    const { push } = useHistory();
    const dispatch = useAppDispatch();
    const [status, setStatus] = useState<EnumStatus>(EnumStatus.ALL);
    const { error, loading, data } = useSelector(({ story: { all } }: IRootState) => all);
    ...
    const title = `Story ${!isStatusAll ? `${format(isStatusActive ? 'actifs' : 'inactifs)', 'lowercase')}` : ''}`;
    useEffect(() => onTitle?.(title), [onTitle, title]);

    const navigateToStatus = (status: EnumStatus): void => {
        if (status === EnumStatus.ALL) push(STORY_PATH);
        else {
            push({
                pathname: STORY_PATH,
        ...
        <Fragment>
            ...
            <Wrapper>
                ...
                <Table
                    ...
                    rowKey={(story: IUnknownObject) => story.id}
                    ...
                    pagination={{
                        ...
                        showTotal: (t) => `${numeral(t).format('0,0')} story`,
